...
def load_configs(config_dir: Path) -> Dict[str, ConfigBundle]:
    if not config_dir.exists() or not config_dir.is_dir():
        LOGGER.error("Config directory %s missing or not a directory", config_dir)
        raise RuntimeError("Config directory not found or not a directory")
    ...
    for path in sorted(config_dir.glob("*.y*ml")):
        ...
        for key, tmpl in fields.items():
            ...
            try:
                ...
                raise RuntimeError(f"Config {path} invalid template for '{key}': {exc}") from exc
        ...
        for idx, rule in enumerate(highlight_rules_raw):
            ...
            if when is None or not isinstance(when, str):
                raise RuntimeError(f"Config {path} highlight rule #{idx} missing 'when' expression")
            if class_name is None or not isinstance(class_name, str):
                raise RuntimeError(f"Config {path} highlight rule #{idx} missing 'class' string")
            try:
    ...
    if not configs:
        raise RuntimeError(f"No config files found in {config_dir}")

    LOGGER.info("Loaded %d config(s) from %s", len(configs), config_dir)
    return configs
